            ...
            <Paper sx={{ mb: 2, p: 2 }}>
                <Toolbar sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 2, padding: 0 }}>
                    ...
                    <FormControl variant="standard" sx={{ minWidth: 150, width: isMobile ? '100%' : 'auto' }}>
                        ...
                        <Select value={stockFilter} onChange={(e) => setStockFilter(e.target.value as string)}>
                    ...
                    {filteredRows.length > 0 ? (
                        filteredRows.map(p => (
                            <Card key={p.id} sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', p: 1 }}>
                                {p.imagen ? (
                                    <CardMedia component="img" sx={{ width: 80, height: 80, borderRadius: 1, flexShrink: 0 }} image={p.imagen} alt={p.titulo} />
                                ) : (
                                    <Avatar variant="rounded" sx={{ width: 80, height: 80, flexShrink: 0 }}><Inventory2Icon /></Avatar>
                                )}
            ...
            <Dialog open={open} onClose={handleClose}>
                ...
                <DialogActions>
                    ...
                    <Button 
                        ...
                        disabled={!newProduct.titulo || !newProduct.precio || !newProduct.stock} // Desactivar si faltan campos requeridos
                    >
                        Guardar
                    </Button>
                </DialogActions>
